...
public static int nb_sec_rem;   ///< remainder of seconds since startup in CPU_TICKS
...
public  static int ticks_per_sec;         ///< sys_time ticks per second (SYS_TIME_FREQUENCY)
...
for (int i = 0; i< SYS_TIME_NB_TIMER; i++) {
  if (!timer[i].in_use) {
    ...
    timer[i].end_time   = start_time + sys_time_ticks_of_sec(duration);
